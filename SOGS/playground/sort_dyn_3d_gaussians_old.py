...
def sort_dyn_gaussians(df, resume_from_last = True, init_order= None, seq="Base", exp="Base"):
    ...
    #remove all fields that are coded in float64 and print( their names)
    for col in sorted_df.columns:
        print(f"Column {col} has dtype {sorted_df[col].dtype}")
        if sorted_df[col].dtype == np.float64:
            print(f"Dropping column {col} of type float64")
