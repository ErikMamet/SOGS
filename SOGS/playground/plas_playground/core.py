...
def params_to_blocky(
    ...
    """Converts 2D parameter grid into blocky format for efficient sorting.
    
    This function reorganizes a 2D grid of parameters into a hierarchical block structure
    to facilitate parallel linear assignment sorting. It performs cyclic shifting followed
    by hierarchical reshaping to group pixels into sortable blocks.
    
    Args:
        params: Tensor of shape (c, h, w) where c=channels, h=height, w=width
        block_size: Size of each block in pixels (assumed square)
        block_divisor: Divisor for hierarchical pixel unshuffling (stride of unshuffle)
        num_pixel_blocks: Number of blocks along each dimension
        shift_y: Cyclic shift amount in y-direction
        shift_x: Cyclic shift amount in x-direction
    
    Returns:
        params_rolled: Original input cyclically shifted by (shift_y, shift_x)
        params_blocky_flat: Blocks in flat format (r^2 * pb^2, c, block_size * block_size)
    """
    # Step 1: Apply cyclic shift to enable different block configurations
    ...
    # (c, h, w) -> (c, num_pixel_blocks*block_size, num_pixel_blocks*block_size)
    ...
    # PixelUnshuffle expects (batch, channels, height, width)
    # (c, h, w) -> (c, 1, h, w)
    params_c_hw = params_truncated.unsqueeze(1)

    # Step 4: Apply PixelUnshuffle to extract spatial neighborhoods
    # PixelUnshuffle(block_divisor) reorganizes r^2 neighborhoods into channel dimension
    # where r = block_divisor
    # (c, 1, h, w) -> (c, r^2, h/r, w/r)
    params_unshuffled_cbhw = torch.nn.PixelUnshuffle(block_divisor)(params_c_hw)

    # Step 5: Move channel dimension back to second position for standard layout
    # (c, r^2, h/r, w/r) -> (r^2, c, h/r, w/r)
    params_unshuffled = params_unshuffled_cbhw.permute(1, 0, 2, 3)

    # Step 6: Hierarchically reshape to separate block and pixel indices
    # Decompose spatial dimensions into (num_pixel_blocks, block_size) pairs
    # This creates the hierarchical structure: [r^2_index, channel, pb_y, py, pb_x, px]
    # (r^2, c, h/r, w/r) -> (r^2, c, pb, py, pb, px)
    # where pb = num_pixel_blocks (pixel block count), py/px = block_size (pixels per block)
    ...
    # Step 7: Rearrange dimensions to group block indices together
    # Move pixel block indices (pb_y, pb_x) before pixel indices (py, px)
    # and channel dimension after r^2 and pb dimensions
    # (r^2, c, pb, py, pb, px) -> (r^2, pb_y, pb_x, c, py, px)
    params_unshuffled_blocky_first = params_unshuffled_blocky_inline.permute(
        0, 2, 4, 1, 3, 5
    )

    # Step 8: Flatten batch dimensions into a single dimension
    # Collapses (r^2, pb_y, pb_x) into a single batch dimension
    # Each unique combination becomes one "block" to be sorted
    # (r^2, pb_y, pb_x, c, py, px) -> (r^2 * pb^2, c, py, px)
    ...
    # Step 9: Flatten pixel spatial dimensions into a single vector per block
    # Each block's pixels are now a 1D feature vector for sorting/comparison
    # (r^2 * pb^2, c, py, px) -> (r^2 * pb^2, c, block_size * block_size)
